...
# API Endpoints
@app.post("/create")
async def create_lobby(host_name: str = Form(...)):
    user = lobby_code.User(host_name)
    id = user.create_lobby()
    if id is None:
        raise HTTPException(status_code=404, detail="SOMETHING IS WRONG")
    return RedirectResponse(url=f"/lobbies/{id}")

@app.get("/lobbies/{lobby_id}") 
async def get_lobby(request: Request, lobby_id: str):
    return templates.TemplateResponse("lobby.html", {"request": request, "lobby_id": lobby_id, "message": "Welcome to your lobby!"})
...
async def start_lobby(lobby_id: str):
    lobby_code.LobbyUtils.start_lobby(lobby_id)
    return RedirectResponse(url=f"/lobbies/{lobby_id}/match")

@app.get("/lobbies/{lobby_id}/match")
async def match_screen(request:Request,lobby_id:str):
    return templates.TemplateResponse("swipe.html", {"request": request, "lobby_id": lobby_id, "message": "Welcome to your lobby!"})

@app.get("/lobbies/{lobby_id}/match/accept")
async def match_screen(lobby_id:str):
    return templates.TemplateResponse("swipe.html", {"request": request, "lobby_id": lobby_id, "message": "Welcome to your lobby!"})
